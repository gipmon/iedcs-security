...
public class Test
{
  ...
  private static void fillPersoFile()
  {
        byte PTEID_AUTH_PIN_ID = (byte)0x81;
        byte[] persoFile = { (byte)0x3F, (byte)0x00, (byte)0x5F, (byte)0x00, (byte)0xEF, (byte)0x07 };
        String[] poema = new String[3];
        ...
        poema[1] = "E tambem as memorias gloriosas\n" +
        ...
        poema[2] = "Cessem do sábio Grego e do Troiano\n" +
        ...
        int countOffset = 0;
        for (String estrofe : poema)
        {
            try {
                //byte[] infile = Encoding.UTF8.GetBytes(estrofe);
                byte[] infile = estrofe.getBytes();
                pteid.WriteFileInOffset(persoFile, infile, PTEID_AUTH_PIN_ID, countOffset);
                countOffset += infile.length;
